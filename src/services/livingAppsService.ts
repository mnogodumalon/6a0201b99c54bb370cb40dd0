...
import type { BdbddbErfassung, CreateBdbddbErfassung } from '@/types/app';
...
export class LivingAppsService {
  // --- BDBDDB_ERFASSUNG ---
  static async getBdbddbErfassung(): Promise<BdbddbErfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BDBDDB_ERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as BdbddbErfassung[];
    return enrichLookupFields(records, 'bdbddb_erfassung');
  }
  static async getBdbddbErfassungEntry(id: string): Promise<BdbddbErfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BDBDDB_ERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as BdbddbErfassung;
    return enrichLookupFields([record], 'bdbddb_erfassung')[0];
  }
  static async createBdbddbErfassungEntry(fields: CreateBdbddbErfassung) {
    return callApi('POST', `/apps/${APP_IDS.BDBDDB_ERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'bdbddb_erfassung') });
  }
  static async updateBdbddbErfassungEntry(id: string, fields: Partial<CreateBdbddbErfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.BDBDDB_ERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bdbddb_erfassung') });
  }
  static async deleteBdbddbErfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BDBDDB_ERFASSUNG}/records/${id}`);
  }
