...
interface BdbddbErfassungViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: BdbddbErfassung | null;
  onEdit: (record: BdbddbErfassung) => void;
}

export function BdbddbErfassungViewDialog({ open, onClose, record, onEdit }: BdbddbErfassungViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Bdbddb Erfassung anzeigen</DialogTitle>
        </DialogHeader>
