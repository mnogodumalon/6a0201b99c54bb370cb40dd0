...
interface BdbddbErfassungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: BdbddbErfassung['fields']) => Promise<void>;
  defaultValues?: BdbddbErfassung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BdbddbErfassungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: BdbddbErfassungDialogProps) {
  const [fields, setFields] = useState<Partial<BdbddbErfassung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'bdbddb_erfassung');
      await onSubmit(clean as BdbddbErfassung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<BdbddbErfassung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Bdbddb Erfassung bearbeiten' : 'Bdbddb Erfassung hinzufügen';
