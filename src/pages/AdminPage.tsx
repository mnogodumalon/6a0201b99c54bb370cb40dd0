...
import { BdbddbErfassungViewDialog } from '@/components/dialogs/BdbddbErfassungViewDialog';
...
const ENTITY_TABS = [
  { key: 'bdbddb_erfassung', label: 'Bdbddb Erfassung', pascal: 'BdbddbErfassung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('bdbddb_erfassung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'bdbddb_erfassung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bdbddb_erfassung': return (data as any).bdbddbErfassung as BdbddbErfassung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bdbddb_erfassung': return BDBDDBERFASSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bdbddb_erfassung': return {
        create: (fields: any) => LivingAppsService.createBdbddbErfassungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBdbddbErfassungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBdbddbErfassungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'bdbddb_erfassung' || dialogState?.entity === 'bdbddb_erfassung') && (
        <BdbddbErfassungDialog
          open={createEntity === 'bdbddb_erfassung' || dialogState?.entity === 'bdbddb_erfassung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bdbddb_erfassung' ? handleUpdate : (fields: any) => handleCreate('bdbddb_erfassung', fields)}
          defaultValues={dialogState?.entity === 'bdbddb_erfassung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['BdbddbErfassung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['BdbddbErfassung']}
        />
      )}
      {viewState?.entity === 'bdbddb_erfassung' && (
        <BdbddbErfassungViewDialog
          open={viewState?.entity === 'bdbddb_erfassung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'bdbddb_erfassung', record: r }); }}
        />
      )}
