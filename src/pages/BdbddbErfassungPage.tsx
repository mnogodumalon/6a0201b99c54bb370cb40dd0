...
export default function BdbddbErfassungPage() {
  const [records, setRecords] = useState<BdbddbErfassung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<BdbddbErfassung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<BdbddbErfassung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<BdbddbErfassung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getBdbddbErfassung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: BdbddbErfassung['fields']) {
    await LivingAppsService.createBdbddbErfassungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: BdbddbErfassung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBdbddbErfassungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Bdbddb Erfassung"
      subtitle={`${records.length} Bdbddb Erfassung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Bdbddb Erfassung suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Bdbddb Erfassung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Bdbddb Erfassung löschen"
